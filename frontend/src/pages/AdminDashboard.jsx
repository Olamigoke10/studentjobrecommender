...
const AdminDashboard = () => {
  ...
  const handleUpdate = async (id) => {
    ...
    setError(null);
    try {
      await adminAPI.updateSkill(id, name);
      cancelEdit();
      await loadSkills();
      await loadStats();
    } catch (err) {
  ...
  return (
    <div className="py-4 sm:py-6 animate-fade-in">
      ...
      <section>
        ...
        <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-6">
          <input
            type="text"
            value={newName}
                    ...
                    {editingId === skill.id ? (
                      <div className="flex flex-col sm:flex-row gap-2 flex-1 min-w-0">
                        <input
                          ...
                          maxLength={100}
                          className="flex-1 px-3 py-2 rounded-lg border border-slate-200 dark:bg-slate-900 dark:border-slate-600 dark:text-slate-100"
                        />
